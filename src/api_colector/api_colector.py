...
class RickMortyApiColector:
    ...
    def info_character_complete(self, id_character: int) -> dict:
        rm_api_response = self.rm_api_consumer.get_character(id_character)
        return self.format_info_character_complete(rm_api_response)
    ...
    def format_info_character_complete(self, character: dict):
        return {
            "name": character.response["name"],
            "status": character.response["status"],
            "image": character.response["image"],
            "species": character.response["species"],
            "gender": character.response["gender"],
            "origin": character.response["origin"]["name"],
            "location": character.response["location"]["name"],
        }
